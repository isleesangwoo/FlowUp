...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("input:text[name='searchWord']").bind("keyup", function(e){
			if(e.keyCode == 13) { // 엔터를 쳤을 경우
				goSearch();
			}
		});
		
		// 검색 시 검색조건 및 검색어 유지시키기
							...
							if(json.length > 0) {
								...
								$.each(json, function(index, item){
									...
									*/
									...
									// word.toLowerCase() 은 word 를 모두 소문자로 변경한다.
									/*
										java가 쉽나요?
										java공부를 하려고 해요. 도와주세요~~
										프로그래밍은 java 를 해야 하나요?
									...
									*/
									...
									/*
										java가 쉽나요?								idx : 0
										java공부를 하려고 해요. 도와주세요~~				idx : 0
										프로그래밍은 java 를 해야 하나요?					idx : 7
										javascript 는 쉬운가요?						idx : 0
										프론트 엔드를 하려면 javascript 를 해야 하나요?		idx : 12
										질문있어요 jquery 와 javascript 는 관련이 있나요?	idx : 15
									*/
								...
								/*	
									...
									console.log(word.substring(idx+len));		// 검색어(JavA) 뒤부터 끝까지의 글자 	==>  를 해야 하나요?
									console.log("~~~ 끝 ~~~");
								*/
		...
		$(document).on("click", "span.result", function(e){
			...
			$("input[name='searchWord']").val(word); // 텍스트박스에 검색된 결과의 문자열을 넣어준다.
			$("div#displayList").hide();
			goSearch();
	...
	<%--
		frm.method = "get"; // default : get
		frm.action = "<%= ctxPath%>/board/list"; // default : 자기 자신
	--%>
		frm.submit();
