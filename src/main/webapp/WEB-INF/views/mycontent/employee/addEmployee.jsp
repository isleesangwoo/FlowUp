	...
	$("input:password[name='passwd']").keyup(function(e){
 	...
 	<%-- ============ 전화번호 유효성 시작 ============ --%>
 	 $("input:text[name='mobile']").keyup(function(e){
		 ...
		 if(current_val.lengt > 11){
			 current_val = current_val.substring(0, 11);
		 }
		 
		 let formattedMobile = current_val;
		 
		 if(formattedMobile.length > 3){
			 formattedMobile = current_val.substring(0, 3)+"-"+current_val.substring(3);
		 }
		 
		 if(formattedMobile.length > 7){
			 formattedMobile = current_val.substring(0,3)+"-"+current_val.substring(3,7)+"-"+current_val.substring(7);
		 }
		 
		 $(e.target).val(formattedMobile);
		 
		 if(current_val.length > 0 && current_val.length < 10){
				$(e.target).next().show();
		 }
...
<div class="container">
	...
	<!-- 		<input type="text" class="frmInput deptNoInfo" name="FK_departmentNo" placeholder="6자리 숫자 조합" maxlength="6"/> -->
			...
			<input type="date" class="frmInput registerInfo" name="registerDate"  />
			<span class="error">'-'없이 숫자로 입력하세요</span>
			</li>
			...
			<input type="date" class="frmInput birthInfo"  name="birth" />
			<span class="error">'-'없이 숫자로 입력하세요</span>
			</li>
